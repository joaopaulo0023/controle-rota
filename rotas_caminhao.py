...
# =========================
    ...
    # =========================
        ...
        with col2:
            motoristas = pd.read_sql_query("SELECT DISTINCT motorista FROM registros_rota", conn)["motorista"].dropna().tolist()
            motorista_filtro = st.selectbox("Motorista", ["TODOS"] + motoristas)

        # FILTRO REGISTROS
        if motorista_filtro == "TODOS":
            query = """
            SELECT locais.nome_local, registros_rota.data_hora, status, motorista
            FROM registros_rota JOIN locais ON local_id = locais.id
            WHERE data_hora BETWEEN ? AND ? ORDER BY data_hora DESC
            """
            params = (data_inicio.strftime("%Y-%m-%d %H:%M:%S"), data_fim.strftime("%Y-%m-%d %H:%M:%S"))
        else:
            query = """
            SELECT locais.nome_local, registros_rota.data_hora, status, motorista
            FROM registros_rota JOIN locais ON local_id = locais.id
            WHERE motorista=? AND data_hora BETWEEN ? AND ? ORDER BY data_hora DESC
            """
            params = (motorista_filtro, data_inicio.strftime("%Y-%m-%d %H:%M:%S"), data_fim.strftime("%Y-%m-%d %H:%M:%S"))
        ...
        with st.expander("📊 Dashboard"):
            if not registros.empty:
                registros["data_hora"] = pd.to_datetime(registros["data_hora"])
                registros["turno"] = registros["data_hora"].dt.hour.apply(
                    lambda h: "1º" if 6<=h<15 else "2º" if 15<=h<23 else "3º"
                )
                
                col1, col2 = st.columns(2)
                with col1:
                    fig = px.histogram(registros, x="status", title="Status")
                    st.plotly_chart(fig, use_container_width=True)
                with col2:
                    fig2 = px.histogram(registros, x="turno", color="status", barmode="group")
        ...
        with col2:
            st.metric("Motoristas", registros["motorista"].nunique())
        with col3:
            st.metric("No Horário", len(registros[registros["status"].str.contains("NO HORARIO", na=False)]))
        with col4:
            st.metric("Atrasados", len(registros[registros["status"].str.contains("ATRASADO", na=False)]))
